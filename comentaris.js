function enviarComent(){
    ...
    if (comentari.length==0){
        ...
        alert("Hem rebut el vostre comentari, enviat el "+llistaDiesSet[diaSet]+", dia "+dies+" de "+llistaMesos[mes]+" de "+anys+" a les "+hores+":"+minuts+". El teu comentari és:\n'"+comentari+"'\nTindrem el vostre comentari en consideració per a poder seguir millorant aquesta pàgina web.");
        document.getElementById("comenta").value="";
    }
}
...
var limitCaract=600;
Pcaracters.innerText="Tens "+limitCaract+" caràcters disponibles restants";
...
addEventListener("input",function(){
    if(comentar.value.length>=limitCaract){comentar.value=comentar.value.substring(0,limitCaract);
        ...
        Pcaracters.innerHTML="Tens "+caractersRestants+" caràcters disponibles restants";
